...
def detach_module(module, keep_requires_grad=False):
    ...
    **Example**
    ...
    for buffer_key in module._buffers:
        if (
            module._buffers[buffer_key] is not None
            and module._buffers[buffer_key].requires_grad
        ):
    ...
    for module_key in module._modules:
        detach_module(
            module._modules[module_key], keep_requires_grad=keep_requires_grad
        )
...
def clone_distribution(dist):
    ...
    for param_key in clone.__dict__:
        ...
        if isinstance(item, torch.Tensor):
            ...
            clone.__dict__[param_key] = clone_distribution(
                dist.__dict__[param_key]
            )
...
def detach_distribution(dist):
    ...
    for param_key in dist.__dict__:
        ...
        if isinstance(item, torch.Tensor):
            ...
            dist.__dict__[param_key] = detach_distribution(
                dist.__dict__[param_key]
            )
    return dist
